...
│                             用户权限模块                                  │
│  ┌────────┐      ┌──────────┐      ┌────────┐      ┌──────────┐        │
│  │ t_user │──M:N─│t_user_role│──M:N─│ t_role │──M:N─│t_role_perm│       │
│  └────────┘      └──────────┘      └────────┘      └─────┬────┘        │
│       │                                                   │             │
│       │                                          ┌──────────────┐      │
│       └──────────────────────────────────────────│t_permission  │      │
...
│                             任务调度模块                                  │
│  ┌────────┐      ┌──────────┐      ┌───────────┐                       │
│  │ t_job  │──1:N─│ t_job_log│      │t_job_flow │                       │
│  └────────┘      └──────────┘      └─────┬─────┘                       │
│                                          │                              │
│                               ┌──────────────────┐                     │
│                               │ t_job_flow_node  │                     │
...
│                              审批流程模块                                 │
│  ┌─────────────────────┐    ┌───────────────────┐   ┌─────────────────┐│
│  │t_workflow_definition│─1:N│t_approval_instance│1:N│t_approval_record││
│  └─────────────────────┘    └───────────────────┘   └─────────────────┘│
└──────────────────────────────────────────────────────────────────────────┘
...
│                              报表模块                                     │
│  ┌──────────────┐      ┌──────────┐      ┌──────────────────────┐      │
│  │ t_datasource │──1:N─│ t_report │──1:N─│t_report_subscription │      │
│  └──────────────┘      └──────────┘      └──────────────────────┘      │
└──────────────────────────────────────────────────────────────────────────┘
```
...
| 鉴权 | t_role | 角色表 |
| 鉴权 | t_permission | 权限表 |
| 鉴权 | t_user_role | 用户角色关联 |
| 鉴权 | t_role_permission | 角色权限关联 |
| 鉴权 | t_department | 部门表 |
| 调度 | t_job | 任务定义表 |
| 调度 | t_job_flow | 工作流定义表 |
| 调度 | t_job_log | 任务执行日志 |
| 报表 | t_datasource | 数据源配置 |
| 报表 | t_report | 报表定义 |
| 报表 | t_report_subscription | 报表订阅 |
| 配置 | t_meta_table | 元数据表定义 |
| 配置 | t_config_history | 配置历史记录 |
| 审批 | t_workflow_definition | 流程定义 |
| 审批 | t_approval_instance | 审批实例 |
| 审批 | t_approval_record | 审批记录 |
| 审计 | t_audit_log | 操作审计日志 |
| 审计 | t_data_change_log | 数据变更日志 |
| 审计 | t_login_log | 登录日志 |
| 消息 | t_message_template | 消息模板 |
| 消息 | t_message_log | 消息发送记录 |
| 消息 | t_notification | 站内通知 |
...
services:
  web:
    ...
    ports:
      - "8000:8000"
    environment:
      - DATABASE_URL=mysql://user:pass@db:3306/db_ops
